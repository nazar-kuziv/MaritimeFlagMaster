...
class FlagSen(Util.AppQuizPage, Util.ISkippablePage):
    ...
    def draw(self):
        ...
        def save_image():
            if (Alphabet.save_flag_sentence_png(self.question.flags, suggest_file_name=self.is_answered)):
                label = ctk.CTkLabel(self.container_frame, text='Zapisano.',
        ...
        self.save_image = ctk.CTkButton(self.container_frame, text='Zapisz obecne jako zdjęcie...', width=0,
                                        font=ctk.CTkFont(size=int(self.master.winfo_width() * 0.015)),
                                        command=save_image)
    ...
    def establish_session(self, source: str):
        ...
        if (error_text != ""):
            error_message = ctk.CTkLabel(self, text=error_text,
    ...
    def show_question(self):
        ...
        self.answer_response = ctk.CTkLabel(self.container_frame, text='',
                                            font=ctk.CTkFont(size=int(self.master.scale_size * 0.03)),
                                            fg_color='transparent')
        ...
        for i, flag in enumerate(self.question.flags):
            img = None
            img = tksvg.SvgImage(file=Environment.resource_path(flag.img_path),
                                 scaletowidth=int(self.master.scale_size * 0.1)) if (flag is not None) else None
        ...
        self.answer_cell.entry = ctk.CTkEntry(self.answer_cell, width=int(self.master.scale_size * 0.6),
        ...
        self.text_length = ctk.CTkLabel(self.answer_cell, text=f"0/{len(self.question.cleaned_sentence)}",
                                        width=int(self.master.scale_size * 0.05), fg_color='transparent')
        self.text_length.grid(row=0, column=0, sticky="e", padx=10)

        self.answer_cell.submit_button = ctk.CTkButton(self.answer_cell, text='Sprawdź',
                                                       font=ctk.CTkFont(size=int(self.master.scale_size * 0.03)),
                                                       command=self.enter_answer)
