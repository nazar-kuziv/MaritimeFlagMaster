...
class SenFlag(Util.AppQuizPage, Util.ISkippablePage):
    ...
    def establish_session(self, mode: str):
        ...
        if (error_text != ""):
            error_message = ctk.CTkLabel(self.container_menu, text=error_text,
                                         font=ctk.CTkFont(size=int(self.master.scale_size * 0.05)), fg_color='white')
    ...
    def show_question(self):
        ...
        try:
            ...
            print("Couldn't destroy input_frame")
        try:
            self.input_parent.destroy()
        except AttributeError:
            print("Couldn't destroy flag_input_box")
        self.update()
        self.master.scale_size = self.master.winfo_height() if (
                    self.master.winfo_height() < self.master.winfo_width()) else self.master.winfo_width()
        ...
        meaning_label = ctk.CTkLabel(self.top_menu, text=self.question.cleaned_sentence,
                                     width=int(self.master.winfo_width() * 0.3),
                                     font=ctk.CTkFont(size=int(self.master.winfo_width() * 0.011)),
        ...
        self.check_button = ctk.CTkButton(self.top_menu, text="Sprawdź", width=0,
                                          font=ctk.CTkFont(size=int(self.master.winfo_width() * 0.015)),
                                          command=self.check_answer)
        self.check_button.pack(side="left", ipadx=10, ipady=10, padx=10)
        self.top_menu.dict["check_button"] = self.check_button

        self.delete_button = ctk.CTkButton(self.top_menu, text="Kasuj", width=0,
                                           font=ctk.CTkFont(size=int(self.master.winfo_width() * 0.015)),
                                           command=self.delete_input_flag)
        self.delete_button.pack(side="left", ipadx=10, ipady=10, padx=10)
        self.top_menu.dict["delete_button"] = self.delete_button
        ...
        self.answer_response = ctk.CTkLabel(self.top_menu, text="",
                                            font=ctk.CTkFont(size=int(self.master.winfo_width() * 0.015)),
        ...
        self.text_length = ctk.CTkLabel(self.input_parent, text=f"0/{len(self.question.cleaned_sentence)}",
    ...
    def skip_command(self):
        ...
        for f in correct_flags:
            if (f is None):
                new_input_flag = ctk.CTkLabel(self.flag_input_box, text='␣',
    ...
    def show_next_button(self):
        ...
        self.next_button = ctk.CTkButton(self.top_menu, text=next_text,
                                         font=ctk.CTkFont(size=int(self.master.winfo_width() * 0.015)),
